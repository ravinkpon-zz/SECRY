...
def register(request):                      #User registration request and processing
    if request.method == 'POST':
        ...
        if User.objects.filter(username=username).exists():
            ...
            html_message = get_template('acc_active_email.html', {'domain': current_site.domain, 'uid': urlsafe_base64_encode(force_bytes(uid)), 'token': account_activation_token.make_token(user)})
